...
const int TX_ADDRESS[TX_ADR_WIDTH]={0x34,0x43,0x10,0x10,0x01}; //发送地址

const int RX_ADDRESS[RX_ADR_WIDTH]={0x34,0x43,0x10,0x10,0x01}; //接收地址
...
{ 	
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7|GPIO_Pin_8;	//PB8 7 推挽 	  
 	GPIO_Init(GPIOB, &GPIO_InitStructure);//初始化指定IO
...
int NRF24L01_Read_Buf(int reg,int *pBuf,int len)
{
	int status,int_ctr;	       
 	...
 	for(int_ctr=0;int_ctr<len;int_ctr++)pBuf[int_ctr]=SPI2_ReadWriteByte(0XFF);//读出数据
...
int NRF24L01_Write_Buf(int reg, int *pBuf, int len)
{
	int status,int_ctr;	    
 	NRF24L01_CSN = 0;          //使能SPI传输
  	status = SPI2_ReadWriteByte(reg);//发送寄存器值(位置),并读取状态值
  	for(int_ctr=0; int_ctr<len; int_ctr++)SPI2_ReadWriteByte(*pBuf++); //写入数据	 
  	NRF24L01_CSN = 1;       //关闭SPI传输
  	return status;          //返回读到的状态值
}				   
//启动NRF24L01发送一次数据
...
{
	NRF24L01_CE=0;	  
  	NRF24L01_Write_Buf(NRF_WRITE_REG+RX_ADDR_P0,(int*)RX_ADDRESS,RX_ADR_WIDTH);//写RX节点地址
...
{														 
	NRF24L01_CE=0;	    
  	NRF24L01_Write_Buf(NRF_WRITE_REG+TX_ADDR,(int*)TX_ADDRESS,TX_ADR_WIDTH);//写TX节点地址 
  	NRF24L01_Write_Buf(NRF_WRITE_REG+RX_ADDR_P0,(int*)RX_ADDRESS,RX_ADR_WIDTH); //设置TX节点地址,主要为了使能ACK	  
